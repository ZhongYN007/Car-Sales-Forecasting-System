...
def rating_ranking(request):
    """
    评分排行
    """
    return render(request,'assets/rating_ranking.html', locals())

def sales_ranking(request):
    """
    销量排行
    """
    assets = models.Salesrank.objects.all()
    return render(request, 'assets/sales_ranking.html', locals())

def sales_predictions(request):
    """
    销量预测
    """
    return render(request,'assets/sales_predictions.html', locals())
